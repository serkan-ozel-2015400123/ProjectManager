...
--
-- Anamakine: 127.0.0.1
-- Üretim Zamanı: 07 May 2018, 19:49:12
...
ELSEIF (x="ALL") THEN
SET @i =0;
SELECT @i=id FROM project INNER JOIN (SELECT DISTINCT project_id FROM projectmanager_project) t ON t.project_id = id WHERE EXISTS (SELECT * FROM task WHERE project_id = @i) AND NOT EXISTS (SELECT * FROM task WHERE project_id = @i AND date > NOW());

END IF;
END$$

CREATE DEFINER=`root`@`localhost` PROCEDURE `incompleted` (IN `x` VARCHAR(255))  READS SQL DATA
BEGIN
SET @isnum = (SELECT x REGEXP '^[0-9]+$');
IF (@isnum=1) THEN

SELECT id i FROM project WHERE id NOT IN (SELECT id AS i FROM project
INNER JOIN (SELECT DISTINCT project_id FROM projectmanager_project WHERE project_manager_id = CAST(x AS UNSIGNED)) t ON t.project_id = id WHERE EXISTS (SELECT * FROM task WHERE project_id = i) AND NOT EXISTS (SELECT * FROM task WHERE project_id = i AND date > NOW()));

ELSEIF (x="ALL") THEN
SET @i =0;
SELECT @i=id FROM project WHERE id NOT IN (SELECT id AS i FROM project WHERE EXISTS (SELECT * FROM task WHERE project_id = @i) AND NOT EXISTS (SELECT * FROM task WHERE project_id = @i AND date > NOW()));
